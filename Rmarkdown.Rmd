...
- [範例資料下載](https://yen0619.github.io/2018_NCCU_R/data/weather.csv)
...
## Rmarkdown 呈現
[範例連結](https://yen0619.github.io/2018_NCCU_R/weatherdemo.html)
<center><img  src="img/exam_q1_1_1.png" width="750" ></center>  
...
```{r include=FALSE}
# Hint for Linu& Mac：
dat <- read.csv("data/weather.csv", fileEncoding="utf8") 
max(dat[1:2, 4:5])
min(dat[1:2, 4:5])
...
# day3 %>% filter(早晚=="白天") %>% mutate(溫度=paste(高溫,低溫,sep="-")) %>% select(天氣, 溫度)
night1 <- filter(dat, 早晚=="晚上")
night2 <- mutate(night1, 溫度=paste(高溫,低溫,sep="-"))
night3 <- select(night2, 天氣, 溫度)
# night3 %>% filter(早晚=="晚上") %>% mutate(溫度=paste(高溫,低溫,sep="-")) %>% select(天氣, 溫度)
...
- 以上參考解答code的產出參考報告
- [參考示範連結](https://yen0619.github.io/2018_NCCU_R/weatherdemo.html)
...
##  目錄
- Rmarkdown 有提供[目錄](http://rpubs.com/yen0619/448059)的功能
- 文件長時方便尋找
...
- 報告內圖片的長寬可以變動：透過`fig_width`以及`fig_height: 7.5`參數調整
